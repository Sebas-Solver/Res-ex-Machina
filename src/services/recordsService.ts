...
/**
 * Verifica que no existan duplicados de wallet+nonce, ni fee_tx_hash.
 * El chequeo de content_hash se realiza de manera independiente en PaymentVerifier.preCheck.
 * @throws ApiError 409 (duplicate_nonce | fee_tx_reused)
 */
...
export async function createRecord(
    ...
    try {
        await db.insert(records).values({
            ...
            feeAmount: attempt.amountAtomic ? formatUnits(BigInt(attempt.amountAtomic), attempt.currency === 'USDC' ? 6 : 18) : "0.00000000",
            feeCurrency: attempt.currency || "ETH",
            feeTxHash: attempt.txHash || null,
            feeBlock: attempt.__legacyFeeData?.feeBlock ?? null,
            feeConfirmedAt: attempt.__legacyFeeData?.feeConfirmedAt ?? null,
